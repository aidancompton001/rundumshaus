...
**Уточнение при исполнении (13.09).** Структура шаблона — по тексту Кевина, а не по `*stadtseiten_fields`: `intro[]`, разделы `{heading, paragraphs, listIntro?, items?, paragraphsAfter?}` в `sectionsBefore/After/End`, `leistungen` (строки 27–47, из него же берут позиции `ServiceDetail`), `firma` (строки 125–137: `hqItems` — список Osnabrück, на других городах город + до 7 соседей), `cta` (154–162), `faq.items[].aOtherCities` (ответ 183 на других городах: «in {city}, {nachbarn}»). В админке — своя коллекция полей. Запись в `services.json` сделана здесь (перенос из Ф2). Критерий приёмки `T012-F1-NO-DEAD-URLS` («0 страниц без шаблона») заменён на `T012-F3-PAGES`.
...
**Уточнение при исполнении (13.09).** `/einsatzgebiet/` услуг не показывает вовсе: города там простым текстом по просьбе Кевина (PX-058), `SERVICE_ICONS` в разметку не выводится — услуга добавлена в список для полноты, в проверку по сборке эта страница не входит. Найдено попутно, не исправлено (вне скоупа): на `/osnabrueck/` у «Garten- und Landschaftsbau» подпись «Kostenlose Abholung von Altmetall im Stadtgebiet.» — хвост от Schrottabholung (PX-077).
...
**Уточнение при исполнении (13.09).** Пункт 1 закрыт по ходу Ф1–Ф4 (тесты правились вместе с кодом). Для тестов пункта 2 код уже существовал, поэтому RED доказан подлогом дефекта в коде (скрипт возвращает файл): убрать услугу из `CITY_PAGE_SERVICE_IDS`, добавить Nordhorn в noindex, убрать ссылку из шаблона Hausmeister, выкинуть услугу из формы — каждый раз падает ровно нужный тест. В форме вариантов 7, а не 8: 6 услуг + «Sonstiges» (в roadmap ошибка счёта). Проверки пунктов 3–4 — `verify/check_t012_nordhorn.py` и `verify/check_t012_verbatim.py`, режут куски из собранного HTML; `--mutate-126` и `--mutate-word` дают код 1.
